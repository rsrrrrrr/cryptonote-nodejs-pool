...
let submitBlock = config.rpcMethods == null ? 'submitblock' : config.rpcMethods.submitblock.key
let submitData = config.rpcMethods == null ? '[{0}]' : config.rpcMethods.submitblock.value
...
function processShare(miner, job, blockTemplate, params){
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        apiInterfaces.rpcDaemon(submitBlock, JSON.parse(utils.StringFormat(submitData, [shareBuffer.toString('hex')])), function(error, result){
            if (error){
