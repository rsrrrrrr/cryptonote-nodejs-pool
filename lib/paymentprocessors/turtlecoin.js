...
function send_prepared_transaction_advanced(createTransactionResult, transferCmd, callback) {
    let tx_hash = "";
    let timeOffset = 0;
    let notify_miners = [];
    if (!(createTransactionResult && createTransactionResult.binary_transaction)) {
        ...
        apiInterfaces.rpcWallet(rpcCommand, rpcRequest, function(error, result){
            ...
            let txHash = daemonType === "bytecoin" ? (tx_hash ? tx_hash : result.transactionHash) : result.tx_hash;
