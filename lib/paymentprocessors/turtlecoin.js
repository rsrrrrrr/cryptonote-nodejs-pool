...
let logSystem = 'payments - turtlecoin';
require('../exceptionWriter.js')(logSystem);

function prepare_transaction_advanced(transferCmd, callback) {
...
//TODO: Make sure amount + fee is available to spend



    apiInterfaces.jsonHttpRequest(config.wallet.host, config.wallet.port, JSON.stringify(transferCmd.rpc), function(error, result){
	if(error) {
	    log('error', logSystem, `Error with ${endpoint} request to WalletApi, ${error.errorMessage}`);
	    return callback(error)
	}
    	transferCmd.rpc.transactionHash = result.transactionHash
    	transferCmd.rpc.fee = result.fee

        if(config.payments.dynamicTransferFee && config.payments.minerPayFee){
            transferCmd.redis.push(['hincrby', config.coin + ':workers:' + transferCmd.worker, 'balance', -transferCmd.rpc.fee]);
        }
        transferCmd.redis.push(['hincrby', config.coin + ':workers:' + transferCmd.worker, 'paid', transferCmd.rpc.amount + transferCmd.rpc.fee]);
        //transferCommands.amount += amount;
...
function send_prepared_transaction_advanced(prepareTransactionResult, transferCmd, callback) {
    let tx_hash = "";
    let timeOffset = 0;
    let notify_miners = [];
    if (!(prepareTransactionResult && prepareTransactionResult.transactionHash)) {
        callback(prepareTransactionResult);
    } else {
    	let endpoint = '/transactions/send/prepared' 
        rpcRequest = {
            transactionHash: prepareTransactionResult.transactionHash
        };
        apiInterfaces.jsonHttpRequest(config.wallet.host, config.wallet.port, rpcRequest, function(error, result){
            if (error){
                log('error', logSystem, 'Error with %s RPC request to wallet daemon %j', [endpoint, error]);
...
function processPayments(balances, minPayoutLevel, callback) {
    ...
    for (let worker in payments){
        ...
        if(!transferCommands[commandIndex]) {
            transferCommands[commandIndex] = {
                ...
                rpc: {
                    destinations: [],
                    feePerByte: config.payments.transferFee,
    ...
    async.filter(transferCommands, function(transferCmd, cback){
        ...
        rpcRequest = {
            ...
            feePerByte: transferCmd.rpc.feePerByte,
            // anonymity: ringSize,
            unlockTime: transferCmd.rpc.unlock_time,
            sourceAddresses: [config.poolServer.pooAddress]
        };
        // if (transferCmd.rpc.payment_id) {
        ...
        async.waterfall([
            async.apply(prepare_transaction_advanced, transferCmd),
            send_prepared_transaction_advanced
        ], function(err, res){
            if (err) {
